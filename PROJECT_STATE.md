...
- Добавлена `ideas/PERMISSIONS.md`: тонкие allow/deny правила для cog/команд, приоритеты user/channel/role/server, global/server ACL, canrun/explain, YAML import/export, replace/update режимы, защита владельца от lockout и загрузка правил для ещё не загруженных модулей.
- Добавлена `ideas/ECONOMY_ADVANCED.md`: role-based PayDay, отдельные cooldown, slot machine с таблицей комбинаций и настраиваемыми min/max ставками, DM таблицы выплат, автоматическая коррекция максимального баланса и global/server режим экономики.
- Добавлена `ideas/GAMES_TRIVIA_ADVANCED.md`: объединение нескольких наборов вопросов, пользовательские YAML-листы с валидацией, CONFIG-переопределения, metadata наборов, победа по max score, отдельные таймеры вопроса/бездействия, режим игры бота, spoiler/reveal UX, гибкое распознавание ответов, выплаты только при 3+ участниках с делением при ничьей, постоянная статистика и независимые сессии по каналам.
- Добавлена `ideas/INTEGRATIONS_STREAMS_ADVANCED.md`: мультиплатформенный мониторинг Twitch/YouTube/Picarto/Kick, несколько независимых alerts, background polling с переходами live/offline, auto-delete offline alerts, ignore reruns/schedules, @everyone/@here/role mentions, кнопка просмотра стрима, API cooldown, OAuth refresh, credential migration, одноразовые owner warnings и устойчивость polling к API errors.
- Добавлены Red-механики mute в `ideas/MODERATION.md`: role-based mute, channel-only mute, timed automatic unmute, группировка нескольких channel mute, отдельный notification channel, DM-уведомления, защита Administrator и ожидание готовности async moderation state.
- После добавления mute-механик файл `ideas/MODERATION.md` был восстановлен из предыдущего состояния и дополнен новыми Red-пунктами; текущая версия сохранена в GitHub.
- В рамках дальнейшего обхода подробно просмотрены cleanup, moderation slowmode, kick/ban, modlog, moderation events/settings, core commands, permissions, trivia, trivia session, streams и mute subsystem.
- Core дополнительно подтверждён как отдельная большая поверхность: global allowlist/blocklist, диагностика команд и traceback, управление модулями/cogs, prefix management и RPC lifecycle требуют дальнейшего разбора на уникальные идеи.
- Важно: Red **не считается завершённым**. Следующий проход продолжает core/downloader/audio/filter/userinfo/admin/events/data/config/i18n/tests/docs/deployment и оставшиеся cogs.
