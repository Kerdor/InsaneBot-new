...
- Добавлена `ideas/GAMES_TRIVIA_ADVANCED.md`: объединение нескольких наборов вопросов, пользовательские YAML-листы с валидацией, CONFIG-переопределения, победа по max score, отдельные таймеры вопроса/бездействия, режим игры бота, spoiler/reveal UX, гибкое распознавание ответов, выплаты только при 3+ участниках с делением при ничьей, постоянная статистика и независимые сессии по каналам.
...
- В рамках дальнейшего обхода подробно просмотрены cleanup, moderation slowmode, kick/ban, modlog, moderation events/settings, core commands, permissions, trivia и streams.
