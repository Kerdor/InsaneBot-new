...
## Дополнительный подтверждённый батч

- **GAB-PUB-081 — 8ball simulated thinking delay:** перед ответом 8ball намеренно выдерживается случайная задержка из заранее заданного набора, создавая эффект «размышления».
- **GAB-PUB-082 — 8ball answer-specific presentation:** случайный ответ хранит собственный цвет/тип оформления, поэтому разные ответы могут визуально различаться.
- **GAB-PUB-083 — About runtime identity:** команда about выводит фактическое имя бота, версию и branch текущего runtime.
- **GAB-PUB-084 — About optional resource links:** ссылки на hosting/help показываются только если соответствующие настройки реально существуют.
- **GAB-PUB-085 — AFK server-local state:** публичный AFK хранится отдельно для каждого пользователя в контексте конкретного сервера.
- **GAB-PUB-086 — AFK current-state query:** вызов AFK без аргумента не изменяет состояние, а показывает текущую настройку или empty-state.
- **GAB-PUB-087 — Alert structured admin escalation:** alert превращается в структурированное сообщение для Bot Admins с автором, каналом, сервером и avatar thumbnail.
- **GAB-PUB-088 — Alert confirmation decoupling:** пользователю возвращается отдельное подтверждение, что администраторы уведомлены, независимо от внутреннего формата admin message.
- **GAB-PUB-089 — Random animal progress stage:** команды случайных животных сначала показывают отдельное сообщение ожидания, а затем результат.
- **GAB-PUB-090 — Random animal no-result tolerance:** если внешний animal provider не вернул картинку без исключения, команда не падает и не публикует мусорный результат.
- **GAB-PUB-091 — Cat/Dog provider symmetry:** одинаковый UX-пайплайн может использоваться для разных типов случайного контента при смене provider method.
- **GAB-PUB-092 — Fact count inherits server fetch policy:** `catfact` и `dogfact` используют общие серверные default/max limits для числа внешних результатов.
- **GAB-PUB-093 — Fact batch pagination:** пачка внешних фактов преобразуется в отдельные страницы без создания множества сообщений.
- **GAB-PUB-094 — Choose quoted options:** выбор случайного варианта поддерживает quoted arguments, позволяя вариантам содержать пробелы.
- **GAB-PUB-095 — Choose alternate separator:** варианты можно разделять `|`, а не только пробелами.
- **GAB-PUB-096 — Roll reversed bounds normalization:** если пользователь указал диапазон в обратном порядке, min/max автоматически меняются местами.
- **GAB-PUB-097 — Roll flexible arity:** один аргумент означает верхнюю границу, два аргумента — полный диапазон; без аргументов используется классический 1–6.
- **GAB-PUB-098 — Convert syntax normalization:** конвертер принимает несколько эквивалентных форматов с необязательным словом `to` и компактной записью значения+единицы.
- **GAB-PUB-099 — Convert typed backend errors:** ошибки currency/unit conversion классифицируются по типу и получают разные подсказки пользователю.
- **GAB-PUB-100 — Weekly games leaderboard:** статистика игр хранится как недельный срез, сортируется по времени и показывает top 8 плюс общее число игр и суммарное время.
- **GAB-PUB-101 — Points bot exclusion:** экономика очков явно запрещает наличие очков у bot accounts.
- **GAB-PUB-102 — Points positive-only leaderboard:** общий рейтинг показывает только участников с количеством очков больше нуля и ограничивает выдачу top 10.
- **GAB-PUB-103 — Points target lazy user document:** при запросе очков пользователя его глобальный документ создаётся лениво, если его ещё нет.
- **GAB-PUB-104 — Timezone validation via timezone database:** команда времени проверяет введённую зону через базу IANA, а не принимает произвольную строку.
- **GAB-PUB-105 — Time self-service fallback:** без timezone команда показывает локальное время runtime и сразу подсказывает формат запроса для конкретной зоны.
- **GAB-PUB-106 — Weather capability availability gate:** погодная команда сначала проверяет наличие API token; при его отсутствии функция считается недоступной, а не падает на сетевом запросе.
- **GAB-PUB-107 — Weather sparse-field rendering:** облачность, дождь, снег и ветер добавляются в embed только когда соответствующие данные присутствуют.
- **GAB-PUB-108 — Weather provider icon integration:** состояние погоды сопровождается provider icon, полученной из кода weather condition.
- **GAB-PUB-109 — Year exact countdown + humanized summary:** команда показывает одновременно точный countdown по дням/часам/минутам/секундам и краткий humanized вариант.
