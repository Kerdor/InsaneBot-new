...
**2.1.** `B1-CORE` — **Runtime information / diagnostics** — единый слой информации о состоянии и окружении бота для диагностики.

**2.2.** `B1-CORE` — **Multi-instance lifecycle** — управление жизненным циклом нескольких экземпляров бота/воркеров.

**2.3.** `B1-CORE` — **Core RPC** — внутреннее взаимодействие между экземплярами/процессами бота.

**2.4.** `B1-CORE` — **Localization / timezone infrastructure** — единые локаль, язык и часовой пояс для серверов/пользователей.
...
**15.1.** `B1-STORAGE` — **Backup / restore** — резервное копирование и восстановление persistent data.
